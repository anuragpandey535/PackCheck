...
// GET all stored inspection records
app.get(['/api/inspections', '/inspections'], async (req, res) => {
  try {
...
app.post(['/api/inspections', '/inspections'], async (req, res) => {
  try {
...
app.delete(['/api/inspections/:id', '/inspections/:id'], async (req, res) => {
  try {
    const { id } = req.params;
    const current = await getStoredInspections();
...
app.post(['/api/analyze-package', '/analyze-package'], async (req, res) => {
  try {
...
if (!process.env.VERCEL) {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`PackCheck API Server running on port ${PORT}`);
  });
}
